...
class CheckOptions(BaseModel):
    ...
    mode: Literal["best_quality", "hybrid", "fast"] = Field(
        ...
        description="Checking mode: best_quality (LLM), hybrid (LLM+LanguageTool), fast (LanguageTool only)"
    )
    returnCorrectedHtml: bool = Field(
        default=False,
        description="Whether to return corrected HTML"
    )
    maxSuggestions: int = Field(
        default=5,
        description="Maximum number of suggestions per issue"
    )
...
class GrammarCheckRequest(BaseModel):
    ...
    contentType: Literal["text/html"] = Field(
        default="text/html",
        description="Content type (currently only text/html supported)"
    )
    language: str = Field(
        default="en-US",
    ...
    options: CheckOptions = Field(
        default_factory=CheckOptions,
        description="Optional checking options"
    )
...
class GrammarIssue(BaseModel):
    ...
    type: Literal["grammar", "spelling", "style"] = Field(
        ...,
    ...
    message: str = Field(
        ...,
        description="Human-readable detailed message"
    )
    shortMessage: str = Field(
        ...,
        description="Short label for the issue"
    )
    plainRange: PlainRange = Field(
        ...,
        description="Character range in plainText"
    )
    context: str = Field(
        ...,
        description="Snippet around the error"
    )
    ...
    replacement: Optional[str] = Field(
        default=None,
        description="Single best replacement (if available)"
    )
...
class GrammarCheckResponse(BaseModel):
    ...
    requestId: Optional[str] = Field(
        default=None,
        description="Request ID (echoed from request or generated)"
    )
    detectedLanguage: str = Field(
        ...,
        description="Detected language code"
    )
    plainText: str = Field(
        ...,
        description="Extracted visible text from HTML"
    )
    issues: List[GrammarIssue] = Field(
        default_factory=list,
        description="List of detected issues"
    )
    stats: ResponseStats = Field(
        ...,
        description="Response statistics"
    )
    correctedHtml: Optional[str] = Field(
        default=None,
        description="Corrected HTML (if returnCorrectedHtml=true)"
    )
